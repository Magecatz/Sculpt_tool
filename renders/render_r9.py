"""R9 capstone render: full Tech Set retargeted across all three bases via
the CORRECTED pipeline (armature placement position+scale + conform). Same
setup as the earlier R6 capstone -- directly comparable, now positioned and
scaled correctly. Regression measures: 9/9 usable; retargeted Top lands
0.39% of body-diagonal from the manual Example1 fit (vs 0.67% ground truth)."""
import sys
...
def fit_piece(gobjs, obj_name, src_body, base_body, base_rig, color):
...
def retarget(base_fbx, base_obj, xoff):
    ...
    gobjs = []
    fit_piece(gobjs, "Sweater by Vinuzhka", src_body, base_body, base_rig, TOPC)
    fit_piece(gobjs, "pants by Vinuzhka", src_body, base_body, base_rig, PANTC)
    for o in src:
        bpy.data.objects.remove(o, do_unlink=True)
...
R.add_label("R1-R9 complete: full Tech Set retargeted -- placed, scaled & conformed to each base",
...
R.add_label("Regression: 9/9 usable; retarget lands 0.39% of body-diagonal from the manual fit",
            (center.x, mins.y - 0.3, mins.z - 0.28), size=0.045, color=(0.7,0.85,0.75))

R.frame_camera(meshes, azimuth_deg=0, elevation_deg=6, zoom=1.1)
out = Path(__file__).parent / "out" / "r9_feature_complete.png"
